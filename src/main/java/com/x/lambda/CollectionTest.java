...
public class CollectionTest {
	public static void main(String[] args) {
		...
		//group by可以根绝 自由分组 
		Map<Integer, List<Integer>> collect2 = numbers.stream().collect(Collectors.groupingBy(x -> x));
...
//		partition只能分成对或者错误的
		Map<Boolean, List<Integer>> collect3 = numbers.stream().collect(Collectors.partitioningBy(x -> x ==1));
